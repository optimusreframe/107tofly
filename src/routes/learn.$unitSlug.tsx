...
function LearnUnit() {
  ...
  if (summary) {
    const maxLevel = 5;
    const avgLevel = summary.masteryDeltas && summary.masteryDeltas.length
      ? summary.masteryDeltas.reduce((s, d) => s + d.level, 0) / summary.masteryDeltas.length
      : 0;
    const avgPct = Math.round((avgLevel / maxLevel) * 100);
    return (
      <StudentAppShell>
          ...
          <Card className="p-6 space-y-4">
            ...
            {summary.masteryDeltas && summary.masteryDeltas.length > 0 && (
              <div className="space-y-2 pt-2">
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Mastery after session</div>
                <div className="h-2 w-full rounded-full bg-muted overflow-hidden">
                  <div className="h-full bg-primary transition-all" style={{ width: `${avgPct}%` }} />
                </div>
                <div className="text-xs text-muted-foreground">Average {avgPct}% across {summary.masteryDeltas.length} concept{summary.masteryDeltas.length === 1 ? "" : "s"}</div>
              </div>
            )}
          ...
          <div className="flex justify-between items-center gap-2">
            <Button variant="ghost" size="sm" className="text-muted-foreground"
              onClick={async () => {
                ...
                try { await reportFn({ data: { exerciseId: current.id, unitId: unit.id, note } }); toast.success("Thanks — reported"); }
